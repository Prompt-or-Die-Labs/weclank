...
// Streaming-platform brand glyphs. Path data fetched from simple-icons.org
// (CC0/MIT-licensed) and embedded verbatim. Rendered as filled shapes inside
// a 24x24 viewBox — single-color via currentColor. Drawn as <path fill="currentColor"
// stroke="none"> rather than the outlined-stroke style used by the action
// icons above.
const brandSvg = (paths: string, size: number): string =>
	`<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="currentColor" stroke="none" aria-hidden="true">${paths}</svg>`;
...
export const Brands = {
	twitch:    (s = 16) => brandSvg(`<path d="${BRAND_PATHS.twitch}"/>`, s),
	youtube:   (s = 16) => brandSvg(`<path d="${BRAND_PATHS.youtube}"/>`, s),
	facebook:  (s = 16) => brandSvg(`<path d="${BRAND_PATHS.facebook}"/>`, s),
	kick:      (s = 16) => brandSvg(`<path d="${BRAND_PATHS.kick}"/>`, s),
	rumble:    (s = 16) => brandSvg(`<path d="${BRAND_PATHS.rumble}"/>`, s),
	x:         (s = 16) => brandSvg(`<path d="${BRAND_PATHS.x}"/>`, s),
	tiktok:    (s = 16) => brandSvg(`<path d="${BRAND_PATHS.tiktok}"/>`, s),
	instagram: (s = 16) => brandSvg(`<path d="${BRAND_PATHS.instagram}"/>`, s),
	linkedin:  (s = 16) => brandSvg(`<path d="${BRAND_PATHS.linkedin}"/>`, s),
};
